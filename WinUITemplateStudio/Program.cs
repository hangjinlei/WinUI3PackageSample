using Microsoft.AspNetCore.Builder;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.Extensions.Hosting;
using Microsoft.UI.Dispatching;
using Microsoft.UI.Xaml;
using Microsoft.Windows.AppLifecycle;
using System.Diagnostics;
using System.Runtime.InteropServices;
using WinUITemplateStudio.Activation;
using WinUITemplateStudio.Contracts.Services;
using WinUITemplateStudio.Core.Contracts.Services;
using WinUITemplateStudio.Core.Services;
using WinUITemplateStudio.Helpers;
using WinUITemplateStudio.Models;
using WinUITemplateStudio.Notifications;
using WinUITemplateStudio.Services;
using WinUITemplateStudio.ViewModels;
using WinUITemplateStudio.Views;

namespace WinUITemplateStudio;

public class Program
{
    public static IHost? Host;
    public static MainWindow? MainWindow;
    public static App? App;

    // https://learn.microsoft.com/windows/apps/windows-app-sdk/applifecycle/applifecycle-single-instance
    [STAThread]
    static int Main(string[] args)
    {
        AppDomain.CurrentDomain.UnhandledException += (s, e) =>
        {
            Environment.Exit(-1);
        };

        AppDomain.CurrentDomain.ProcessExit += (_, __) =>
        {
            try
            {
                using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(5));
                Host?.StopAsync(cts.Token).GetAwaiter().GetResult();
            }
            catch (Exception e)
            {
                // TODO: log
            }
            finally
            {
                Host?.Dispose();
            }
        };

        WinRT.ComWrappersSupport.InitializeComWrappers();
        bool isRedirect = DecideRedirection();

        if (!isRedirect)
        {
            Application.Start(async (p) =>
            {
                try
                {
                    var context = new DispatcherQueueSynchronizationContext(DispatcherQueue.GetForCurrentThread());
                    SynchronizationContext.SetSynchronizationContext(context);

                    var builder = WebApplication.CreateBuilder(args);
                    builder.Host.UseContentRoot(AppContext.BaseDirectory);

                    // Default Activation Handler
                    builder.Services.AddTransient<ActivationHandler<LaunchActivatedEventArgs>, DefaultActivationHandler>();

                    // Other Activation Handlers
                    builder.Services.AddTransient<IActivationHandler, AppNotificationActivationHandler>();

                    // Services
                    builder.Services.AddSingleton<IAppNotificationService, AppNotificationService>();
                    builder.Services.AddSingleton<ILocalSettingsService, LocalSettingsService>();
                    builder.Services.AddSingleton<IThemeSelectorService, ThemeSelectorService>();
                    builder.Services.AddTransient<INavigationViewService, NavigationViewService>();

                    builder.Services.AddSingleton<IActivationService, ActivationService>();
                    builder.Services.AddSingleton<IPageService, PageService>();
                    builder.Services.AddSingleton<INavigationService, NavigationService>();

                    // Core Services
                    builder.Services.AddSingleton<ISampleDataService, SampleDataService>();
                    builder.Services.AddSingleton<IFileService, FileService>();

                    // Views and ViewModels
                    builder.Services.AddTransient<SettingsViewModel>();
                    builder.Services.AddTransient<SettingsPage>();
                    builder.Services.AddTransient<DataGridViewModel>();
                    builder.Services.AddTransient<DataGridPage>();
                    builder.Services.AddTransient<ContentGridDetailViewModel>();
                    builder.Services.AddTransient<ContentGridDetailPage>();
                    builder.Services.AddTransient<ContentGridViewModel>();
                    builder.Services.AddTransient<ContentGridPage>();
                    builder.Services.AddTransient<ListDetailsViewModel>();
                    builder.Services.AddTransient<ListDetailsPage>();
                    builder.Services.AddTransient<MainViewModel>();
                    builder.Services.AddTransient<MainPage>();
                    builder.Services.AddTransient<ShellPage>();
                    builder.Services.AddTransient<ShellViewModel>();

                    // Configuration
                    builder.Services.Configure<LocalSettingsOptions>(builder.Configuration.GetSection(nameof(LocalSettingsOptions)));


                    builder.Services.AddSingleton<MainWindow>();
                    builder.Services.AddSingleton<App>();

                    Host = builder.Build();
                    await Host.StartAsync();

                    App = Host.Services.GetRequiredService<App>();
                    MainWindow = Host.Services.GetRequiredService<MainWindow>();

                    var appNotificationService = Host.Services.GetRequiredService<IAppNotificationService>();
                    var activationService = Host.Services.GetRequiredService<IActivationService>();
                    var navigationService = Host.Services.GetRequiredService<INavigationService>();

                    appNotificationService.Initialize();
                    appNotificationService.Show(string.Format("AppNotificationSamplePayload".GetLocalized(), AppContext.BaseDirectory));
                    await activationService.ActivateAsync(null);
                    navigationService.NavigateTo(typeof(MainViewModel).FullName!);
                }
                catch
                {
                    // TODO: log
                    Environment.Exit(-1);
                }
            });
        }

        return 0;
    }

    private static bool DecideRedirection()
    {
        bool isRedirect = false;
        AppActivationArguments args = AppInstance.GetCurrent().GetActivatedEventArgs();
        ExtendedActivationKind kind = args.Kind;
        AppInstance keyInstance = AppInstance.FindOrRegisterForKey("WinUITemplateStudio");

        if (keyInstance.IsCurrent)
        {
            keyInstance.Activated += OnActivated;
        }
        else
        {
            isRedirect = true;
            RedirectActivationTo(args, keyInstance);
        }

        return isRedirect;
    }

    [DllImport("kernel32.dll", CharSet = CharSet.Unicode)]
    private static extern IntPtr CreateEvent(
    IntPtr lpEventAttributes, bool bManualReset,
    bool bInitialState, string lpName);

    [DllImport("kernel32.dll")]
    private static extern bool SetEvent(IntPtr hEvent);

    [DllImport("ole32.dll")]
    private static extern uint CoWaitForMultipleObjects(
        uint dwFlags, uint dwMilliseconds, ulong nHandles,
        IntPtr[] pHandles, out uint dwIndex);

    [DllImport("user32.dll")]
    static extern bool SetForegroundWindow(IntPtr hWnd);

    private static IntPtr redirectEventHandle = IntPtr.Zero;

    // Do the redirection on another thread, and use a non-blocking
    // wait method to wait for the redirection to complete.
    public static void RedirectActivationTo(AppActivationArguments args,
                                            AppInstance keyInstance)
    {
        redirectEventHandle = CreateEvent(IntPtr.Zero, true, false, null);
        Task.Run(() =>
        {
            keyInstance.RedirectActivationToAsync(args).AsTask().Wait();
            SetEvent(redirectEventHandle);
        });

        uint CWMO_DEFAULT = 0;
        uint INFINITE = 0xFFFFFFFF;
        _ = CoWaitForMultipleObjects(
           CWMO_DEFAULT, INFINITE, 1,
           [redirectEventHandle], out uint handleIndex);

        // Bring the window to the foreground
        Process process = Process.GetProcessById((int)keyInstance.ProcessId);
        SetForegroundWindow(process.MainWindowHandle);
    }

    private static void OnActivated(object sender, AppActivationArguments args)
    {
        ExtendedActivationKind kind = args.Kind;
    }
}
